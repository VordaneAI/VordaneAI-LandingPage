...
export const Scene02ServiceMap: React.FC<Scene02Props> = ({ progress, onOpenWaitlist }) => {
  const isExpanded = progress >= 0.4;

  const pillars = [
    { name: 'SECURITY', sub: 'MicroVM enclaves & zero-trust boundaries', icon: Shield, color: 'text-rose-400' },
    { name: 'OBSERVABILITY', sub: 'Frame-by-frame telemetry & instant circuit breakers', icon: Eye, color: 'text-[#00C2FF]' },
    { name: 'GOVERNANCE', sub: 'Deterministic IAM policies & governed tool sandboxes', icon: Lock, color: 'text-amber-400' },
  ];
